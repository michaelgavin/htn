...
#'                 data with.
#'              
#' @return df    The edges data for the whole graph.
#' @return data  The vector of TCP numbers for a subsetted graph.
#' 
#' @examples
#' df = extractData(dnet, file="/path/to/file.csv")
#' tcps = extractData(dnet, file="/path/to/file.csv", sub=T, comms=dnet@communities, by=2)
